...
export function CreateBookingDialog({ children }: CreateBookingDialogProps) {
  ...
  // Fetch available locations when type changes
  useEffect(() => {
    if (open && type) {
  ...
  // Fetch available slots when type and location are selected
  useEffect(() => {
  ...
  formData.set('scheduledAt', slot.startTime.toISOString()) // 👈 FIXED
  formData.set('durationMinutes', slot.durationMinutes.toString())
