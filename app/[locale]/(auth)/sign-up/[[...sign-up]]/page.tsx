// app/[locale]/(auth)/sign-up/page.tsx
import { Metadata } from "next";
import { getLocale } from "next-intl/server";
import { Header } from "@/app/[locale]/_components/Header/Header";
import { FooterSection } from "@/app/[locale]/_components/Footer/FooterSection";
import { SignUpClient } from "./SignUpClient";

interface PageProps {
  params: Promise<{ locale: string }>;
}

export async function generateMetadata({
  params,
}: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const isAr = locale === "ar";
  const baseUrl = "https://jahez.online";

  const title = isAr ? "إنشاء حساب | جاهز" : "Sign Up | JAHEZ";
  const description = isAr
    ? "أنشئ حسابك على جاهز للوصول إلى المنتجات الرائجة وخدمات الاستشارة وتتبع الطلبات."
    : "Create your JAHEZ account to access trending products, consulting services, and order tracking.";

  const alternates = {
    canonical: `${baseUrl}/${locale}/sign-up`,
    languages: {
      en: `${baseUrl}/en/sign-up`,
      ar: `${baseUrl}/ar/sign-up`,
    },
  };

  const ogImage = {
    url: `${baseUrl}/images/signup-og.jpg`,
    width: 1200,
    height: 630,
    alt: isAr ? "إنشاء حساب على جاهز" : "Sign Up for JAHEZ",
  };

  return {
    title,
    description,
    alternates,
    robots: {
      index: false,
      follow: false,
    },
    openGraph: {
      title,
      description,
      url: `${baseUrl}/${locale}/sign-up`,
      siteName: isAr ? "جاهز" : "JAHEZ",
      locale: isAr ? "ar_SA" : "en_US",
      type: "website",
      images: ogImage,
    },
    twitter: {
      card: "summary_large_image",
      title,
      description,
      images: ogImage,
    },
  };
}

export default async function SignUpPage() {
  const locale = await getLocale();
  const isAr = locale === "ar";

  return (
    <main className="min-h-screen bg-white dark:bg-slate-950">
      {/* <Header /> */}
      <SignUpClient locale={locale} isAr={isAr} />
      <FooterSection />
    </main>
  );
}