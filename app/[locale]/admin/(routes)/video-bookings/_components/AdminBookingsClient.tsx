...
// UI redesigned to match manage-users / product-requests admin design system.
...
import {
  Plus, Loader2, Video, ExternalLink, Eye, XCircle,
  MoreHorizontal, Calendar, Clock, Search, X, CalendarDays,
  Trash2, UserX, ClipboardCheck, ChevronLeft, ChevronRight,
  RefreshCw, SlidersHorizontal, Filter, Check,
  Link2, KeyRound, Monitor, AlarmClock,
} from 'lucide-react'
  ...
  return (
    <div className="flex gap-1 overflow-x-auto scrollbar-none">
      {STATUS_TABS.map(t => {
        ...
        const cfg = t.id !== 'ALL' ? BOOKING_STATUS_CONFIG[t.id as BookingStatus] : null
        return (
          <button
            key={t.id}
            type="button"
          ...
          >
            {cfg && (
              <span className={cn(
                ...
                isActive ? (cfg.dot ?? 'bg-[#7b57fc]') : 'bg-muted-foreground/40',
              )} />
            )}
            {t.label}
          </button>
      ...
      <DialogContent className="max-w-xl p-0 gap-0 overflow-hidden flex flex-col max-h-[90vh] rounded-2xl border-border/50 shadow-2xl">
        ...
        <div className="flex items-start gap-3.5 px-5 pt-5 pb-4 border-b border-border/40 bg-muted/10 shrink-0">
          <Avatar className="w-10 h-10 shrink-0">
            <AvatarFallback className={cn('text-xs font-semibold', avatarColor(booking.client.email))}>
        ...
        <div className={cn('mx-5 mt-3 rounded-xl border px-3.5 py-3 text-xs leading-relaxed shrink-0', cfg.borderColor)}>
            ...
            <button key={t.id} type="button" onClick={() => setTab(t.id as any)}
              ...
              {'count' in t && t.count !== undefined && t.count > 0 && (
                <span className={cn(
                  ...
                  tab === t.id ? 'bg-[#7b57fc]/15 text-[#7b57fc]' : 'bg-muted/60 text-muted-foreground',
                )}>
                  {t.count}
                </span>
              )}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-xl bg-muted/20 border border-border/30">
                  ...
                  <p className="text-sm font-semibold mt-1 truncate">{booking.client.fullName ?? '—'}</p>
                  <p className="text-xs text-muted-foreground truncate">{booking.client.email}</p>
                </div>
                <div className="p-3 rounded-xl bg-muted/20 border border-border/30">
                  <SectionLabel>Duration</SectionLabel>
                  <p className="text-xl font-bold tabular-nums mt-1">{booking.durationMinutes}<span className="text-xs font-normal text-muted-foreground ml-0.5">min</span></p>
                </div>
              </div>
              <div className="p-3 rounded-xl bg-muted/20 border border-border/30">
                <SectionLabel>Request Notes</SectionLabel>
                <p className="mt-1.5 text-sm text-foreground/80 leading-relaxed whitespace-pre-wrap">
                  {booking.requestNotes ?? <span className="italic text-muted-foreground">None provided</span>}
                </p>
              </div>
              ...
              {booking.scheduledAt ? (
                <div className="rounded-xl border border-border/40 bg-muted/20 p-4 space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold">{fmtDT(booking.scheduledAt)}</p>
                  </div>
                  ...
                  {booking.meetingPassword && (
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      <KeyRound size={11} />
                      <span className="font-mono">{booking.meetingPassword}</span>
                    </div>
                  )}
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center gap-3 py-10 rounded-xl border border-dashed border-border/40">
                  ...
                  <p className="text-xs text-muted-foreground">No time scheduled yet</p>
                  {booking.status === 'REQUESTED' && (
                    <button type="button"
                      onClick={() => onAction('schedule', booking)}
                      className="h-7 px-3 text-xs font-medium rounded-lg bg-[#7b57fc] hover:bg-[#6a48e8] text-white flex items-center gap-1.5 transition-colors">
              ...
              {booking.slot && (
                <div className="rounded-xl border border-border/30 bg-muted/10 p-3">
                  ...
                  <p className="text-xs mt-1.5 font-medium">
                    {fmtDT(booking.slot.startTime)} — {format(new Date(booking.slot.endTime), 'h:mm a')}
                  </p>
                  <p className="text-[10px] text-muted-foreground mt-0.5">{booking.slot.durationMinutes} min</p>
                </div>
              )}
            </div>
          )}
          ...
          {tab === 'ai' && (
            <div className="space-y-3">
              ...
              {!booking.aiSummary && !booking.transcriptUrl && (
                <p className="text-xs text-muted-foreground italic">No AI data available yet</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="px-5 py-3 border-t border-border/40 shrink-0 flex items-center justify-between bg-muted/10">
          <span className="text-[10px] text-muted-foreground font-mono">{booking.id.slice(0, 8).toUpperCase()}</span>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md p-0 rounded-2xl border-border/50 shadow-2xl overflow-hidden gap-0">
        ...
        <div className="px-5 pt-5 pb-4 border-b border-border/40 bg-muted/10 flex items-center gap-3">
          ...
          <div>
            <DialogTitle className="text-sm font-semibold">Schedule Video Call</DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground mt-0.5">
              {booking ? `For ${booking.client.fullName ?? booking.client.email}` : ''}
            </DialogDescription>
          </div>
        </div>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="p-5 space-y-4">
                  ...
                  <SelectContent className="rounded-xl border-border/60 bg-card/95 backdrop-blur-sm shadow-xl">
                    ...
                    {slots.map(s => (
                      <SelectItem key={s.id} value={s.id} className="text-xs">
                        {format(new Date(s.startTime), 'EEE MMM d, h:mm a')} — {format(new Date(s.endTime), 'h:mm a')} ({s.durationMinutes}m)
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            <FormField control={form.control} name="meetingProvider" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                  Platform <span className="font-normal normal-case text-muted-foreground/70">(optional)</span>
                </FormLabel>
                <Select onValueChange={field.onChange} value={field.value ?? ''}>
                  <FormControl>
                    <SelectTrigger className="h-8 text-xs bg-muted/40 border-border/50 rounded-lg focus:border-[#7b57fc]/40 focus:ring-1 focus:ring-[#7b57fc]/20">
              ...
              <FormItem>
                ...
                <FormControl>
                  <div className="relative">
                    <Link2 size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none" />
            ...
            <FormField control={form.control} name="meetingPassword" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                  Password <span className="font-normal normal-case text-muted-foreground/70">(optional)</span>
                </FormLabel>
                <FormControl>
                  <div className="relative">
                    <KeyRound size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none" />
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button type="submit" disabled={isPending || loadingSlots}
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md p-0 rounded-2xl border-border/50 shadow-2xl overflow-hidden gap-0">
        <div className="px-5 pt-5 pb-4 border-b border-border/40 bg-muted/10 flex items-center gap-3">
          ...
          <div>
            <DialogTitle className="text-sm font-semibold">Mark as Completed</DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground mt-0.5">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="p-5 space-y-4">
            <FormField control={form.control} name="transcriptUrl" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                  Transcript URL <span className="font-normal normal-case text-muted-foreground/70">(optional)</span>
                </FormLabel>
                <FormControl>
                  <div className="relative">
                    <Link2 size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none" />
            ...
            <FormField control={form.control} name="aiSummary" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                  AI Summary <span className="font-normal normal-case text-muted-foreground/70">(optional)</span>
                </FormLabel>
                <FormControl>
                  <Textarea placeholder="Summary of the call…" rows={4}
      ...
      <AlertDialogContent className="w-[92vw] sm:max-w-sm p-0 rounded-2xl border-border/60 bg-card overflow-hidden shadow-2xl">
        <div className="flex items-start gap-3.5 px-5 pt-5 pb-4 bg-red-500/6 border-b border-red-400/15">
          ...
          <AlertDialogHeader className="space-y-0.5 text-left p-0">
            <AlertDialogTitle className="text-base font-semibold text-foreground">Cancel Booking</AlertDialogTitle>
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="w-[92vw] sm:max-w-sm p-0 rounded-2xl border-border/60 bg-card overflow-hidden shadow-2xl">
        <div className="flex items-start gap-3.5 px-5 pt-5 pb-4 bg-orange-500/6 border-b border-orange-400/15">
          ...
          <AlertDialogHeader className="space-y-0.5 text-left p-0">
            <AlertDialogTitle className="text-base font-semibold text-foreground">Mark as No Show</AlertDialogTitle>
            <AlertDialogDescription className="text-xs text-muted-foreground">
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl p-0 gap-0 flex flex-col max-h-[90vh] rounded-2xl border-border/50 shadow-2xl overflow-hidden">
        ...
        <div className="px-5 pt-5 pb-4 border-b border-border/40 bg-muted/10 shrink-0 flex items-center gap-3">
          ...
          <div>
            <DialogTitle className="text-sm font-semibold">Availability Slots</DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground mt-0.5">
        ...
        <div className="px-5 pb-4 pt-4 border-b border-border/40 shrink-0">
          <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-3">Add New Slot</p>
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-3 gap-3 items-end">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Duration (min)</label>
              <div className="flex gap-2">
                <Input type="number" {...form.register('durationMinutes')} min="5"
                  className="h-8 text-xs bg-muted/40 border-border/50 rounded-lg focus:border-[#7b57fc]/40 focus:ring-1 focus:ring-[#7b57fc]/20" />
                <button type="submit" disabled={isPending}
                  ...
                  {isPending ? <Loader2 size={12} className="animate-spin" /> : <Plus size={14} />}
                </button>
              </div>
            </div>
          </form>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 relative [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-muted-foreground/20 [&::-webkit-scrollbar-thumb]:rounded-full">
          ...
          {slots.length === 0 && !loading ? (
            <div className="flex flex-col items-center justify-center gap-3 py-10">
              ...
              <p className="text-xs text-muted-foreground">No slots yet. Create one above.</p>
            </div>
          ) : (
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isInactive ? 'opacity-50 border-border/30' : 'border-border/40',
                      isBooked ? 'bg-amber-500/5' : isInactive ? 'bg-muted/10' : 'bg-card',
                    )}
                  >
                    <div className="min-w-0 flex items-center gap-3">
                      <span className={cn(
                        'h-8 w-8 flex items-center justify-center rounded-lg shrink-0',
                        isBooked ? 'bg-amber-500/10' : isInactive ? 'bg-muted/60' : 'bg-[#7b57fc]/10',
                      )}>
                        <AlarmClock size={13} className={cn(
                          isBooked ? 'text-amber-500' : isInactive ? 'text-muted-foreground/50' : 'text-[#7b57fc]',
                        )} />
                      </span>
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <span className="text-[10px] text-muted-foreground">{slot.durationMinutes} min</span>
                          {isBooked ? (
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 border-t border-border/40 shrink-0">
      ...
      <div className="flex items-center gap-1">
        <button type="button" onClick={() => { if (page > 1) go(page - 1) }} disabled={page <= 1}
          className={cn(
        ...
        <div className="hidden sm:flex items-center gap-1">
          {pages.map((p, i) =>
            p === 'ellipsis' ? (
              ...
              <button key={p} type="button" onClick={() => go(p as number)}
                className={cn(
        ...
        <button type="button" onClick={() => { if (page < totalPages) go(page + 1) }} disabled={page >= totalPages}
          className={cn(
            'flex h-8 w-8 items-center justify-center rounded-lg border text-muted-foreground transition-all',
            page < totalPages
              ? 'border-border/60 hover:border-border hover:text-foreground hover:bg-muted/40'
      ...
      <div className="px-4 pt-3.5 pb-3 border-b border-border/40 shrink-0 space-y-2.5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="relative flex-1 min-w-48 max-w-sm">
            ...
            {emailFilter && !loading && (
              <button type="button"
                onClick={() => { setEmailFilter(''); fetchPage() }}
          ...
          <span className="text-xs text-muted-foreground tabular-nums ml-auto hidden sm:block">
            {pagination.totalCount.toLocaleString()} booking{pagination.totalCount !== 1 ? 's' : ''}
          </span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 relative [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-muted-foreground/20 [&::-webkit-scrollbar-thumb]:rounded-full">
        ...
        {bookings.length === 0 && !loading ? (
          <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-8 py-16">
            ...
            <div>
              <p className="text-sm font-semibold text-foreground/70">No bookings found</p>
              ...
              return (
                ...
                >
                  ...
                  <Avatar className="h-8 w-8 shrink-0">
                    <AvatarFallback className={cn('text-xs font-bold', avatarColor(booking.client.email))}>
                      {getInitials(booking.client.fullName, booking.client.email)}
                    </AvatarFallback>
                  </Avatar>
                  ...
                  <div onClick={e => e.stopPropagation()} className="shrink-0">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-44 rounded-xl border-border/60 bg-card/95 backdrop-blur-sm shadow-xl p-1">
                        ...
                        {booking.status === 'REQUESTED' && (
                          <DropdownMenuItem onClick={() => handleAction('schedule', booking)}
                            ...
                            <span className="h-6 w-6 flex items-center justify-center rounded-md bg-[#7b57fc]/10"><Calendar size={12} className="text-[#7b57fc]" /></span>
                            Schedule
                          </DropdownMenuItem>
                        )}
                        ...
                        {(booking.status === 'PROPOSED' || booking.status === 'CONFIRMED') && (
                          <>
                            <DropdownMenuItem onClick={() => handleAction('complete', booking)}
                              ...
                              <span className="h-6 w-6 flex items-center justify-center rounded-md bg-emerald-500/10"><ClipboardCheck size={12} className="text-emerald-500" /></span>
                              Complete
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleAction('noshow', booking)}
                              ...
                              <span className="h-6 w-6 flex items-center justify-center rounded-md bg-orange-500/10"><UserX size={12} className="text-orange-500" /></span>
                              No Show
                            </DropdownMenuItem>
                          </>
                        )}
                        ...
                        {!['COMPLETED', 'CANCELED', 'NO_SHOW'].includes(booking.status) && (
                          <>
                            ...
                            <DropdownMenuItem onClick={() => handleAction('cancel', booking)}
                              ...
                              <span className="h-6 w-6 flex items-center justify-center rounded-md bg-red-500/10"><XCircle size={12} /></span>
                              Cancel
                            </DropdownMenuItem>
                          </>
                        )}
