...
import { Loader2, Package, Ship, Plane, Truck, Zap, AlertCircle } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const FREIGHT_OPTIONS: { value: ShipmentInput["freightType"]; label: string; icon: any }[] = [
...
export function ShipmentFormDialog({
  ...
  useEffect(() => {
    ...
    if (shipment) {
      ...
      if (shipment.client) {
        setClient({ kind: "user", id: shipment.client.id, displayName: shipment.client.fullName || shipment.client.email, email: shipment.client.email, phone: shipment.client.phone });
      } else if (shipment.guestClient) {
        setClient({ kind: "guest", id: shipment.guestClient.id, displayName: shipment.guestClient.fullName, email: shipment.guestClient.email, phone: shipment.guestClient.phone });
      } else {
        setClient(null);
      }
    } else {
  ...
  function setField<K extends keyof ShipmentInput>(key: K, value: ShipmentInput[K]) {
  ...
  async function handleSubmit() {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        toast.success(isEdit ? "Shipment updated" : `Shipment created — tracking code ${(res.data as any).trackingCode ?? ""}`);
        onSaved();
        onOpenChange(false);
      } else {
        setError(res.error);
        ...
        <div className="space-y-6 py-2">
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input value={form.productLink ?? ""} onChange={(e) => setField("productLink", e.target.value)} placeholder="https://…" />
              </div>
              <div className="space-y-1.5">
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input list="origin-list" value={form.originCountry} onChange={(e) => setField("originCountry", e.target.value)} />
              ...
              <div className="space-y-1.5">
                ...
                <Input list="dest-list" value={form.destinationCountry} onChange={(e) => setField("destinationCountry", e.target.value)} placeholder="Yemen / UAE / USA…" />
              ...
              {FREIGHT_OPTIONS.map((opt) => {
                ...
                return (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => setField("freightType", opt.value)}
                    className={cn(
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.carrierName ?? ""} onChange={(e) => setField("carrierName", e.target.value)} placeholder="e.g. DHL, Aramex, local forwarder…" />
            </div>
          </section>
          ...
          <section className="space-y-3 rounded-lg border border-border bg-muted/30 p-3">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={form.trackingSource} onValueChange={(v) => setField("trackingSource", v as any)}>
              ...
              <>
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) => setField("carrierTrackingNumber", e.target.value)}
                    placeholder="e.g. YT2345678901234CN"
                  />
                </div>
                <div className="flex items-center justify-between">
                  <Label className="text-xs text-muted-foreground">Auto-sync enabled</Label>
                  <Switch checked={form.autoSyncEnabled} onCheckedChange={(v) => setField("autoSyncEnabled", v)} />
                </div>
              </>
            )}
          </section>
          ...
          <section className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input type="number" step="0.01" value={form.weightKg ?? ""} onChange={(e) => setField("weightKg", e.target.value ? Number(e.target.value) : null)} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs text-muted-foreground">Volume (CBM)</Label>
              <Input type="number" step="0.001" value={form.volumeCbm ?? ""} onChange={(e) => setField("volumeCbm", e.target.value ? Number(e.target.value) : null)} />
            </div>
          </section>
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input type="number" step="0.01" value={form.productCost} onChange={(e) => setField("productCost", Number(e.target.value) || 0)} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs text-muted-foreground">Shipping Cost</Label>
                <Input type="number" step="0.01" value={form.shippingCost} onChange={(e) => setField("shippingCost", Number(e.target.value) || 0)} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs text-muted-foreground">Customs Fees</Label>
                <Input type="number" step="0.01" value={form.customsFees} onChange={(e) => setField("customsFees", Number(e.target.value) || 0)} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs text-muted-foreground">Other Fees</Label>
                <Input type="number" step="0.01" value={form.otherFees} onChange={(e) => setField("otherFees", Number(e.target.value) || 0)} />
              </div>
            </div>
          ...
          <section className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => setField("estimatedDelivery", e.target.value || null)}
            />
          </section>
        </div>

        <DialogFooter>
          <Button variant="outline" onClick={() => { onOpenChange(false); setError(null); }}>Cancel</Button>
