    ...
    <div className="space-y-5 animate-pulse">
      ...
      <div className="flex gap-1.5 flex-wrap">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="h-8 w-20 rounded-xl bg-muted/30" />
        ))}
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="h-3 w-14 rounded-md bg-muted/20" />
        <div className="flex-1 h-px bg-border/10" />
        <div className="h-3 w-4 rounded-md bg-muted/20" />
      </div>

      {/* Notification item skeletons */}
      <div className="space-y-2">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="rounded-2xl border border-border/10 bg-card/40 p-4 flex items-start gap-3">
