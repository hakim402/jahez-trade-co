...
export default function PaymentsTable({ initialData }: PaymentsTableProps) {
  ...
  const columns: ColumnDef<any>[] = [
    {
      accessorKey: 'clerkPaymentAttemptId',
      header: 'Payment ID',
      cell: ({ row }) => (
        <span className="font-mono text-xs">{row.original.clerkPaymentAttemptId.slice(-8)}</span>
      ),
    },
    {
      accessorKey: 'type',
    ...
    },
    {
      ...
      header: 'Related Subscription',
      cell: ({ row }) => {
        const sub = row.original.subscription
        const subItem = row.original.subscriptionItem
        if (sub) {
          return <span className="font-mono text-xs">{sub.clerkSubscriptionId.slice(-8)}</span>
        } else if (subItem?.subscription) {
          return <span className="font-mono text-xs">{subItem.subscription.clerkSubscriptionId.slice(-8)}</span>
        }
        return '—'
      },
    },
