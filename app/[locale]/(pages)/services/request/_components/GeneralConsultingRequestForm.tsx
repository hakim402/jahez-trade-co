...
// app/[locale]/(pages)/services/request/_components/GeneralConsultingRequestForm.tsx
...
import {
  getActiveConsultingServices,
  submitGeneralConsultingRequest,
} from "../actions";
...
export function GeneralConsultingRequestForm({ isAr, locale }: Props) {
  ...
  useEffect(() => {
    getActiveConsultingServices().then(setServices);
  }, []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!selectedServiceId) {
      toast.error(isAr ? "يرجى اختيار خدمة" : "Please select a service");
      return;
    }
    ...
    if (!selectedService) {
      toast.error(isAr ? "الخدمة غير صالحة" : "Invalid service selected");
      return;
    }

    startTransition(async () => {
      const result = await submitGeneralConsultingRequest({
        serviceId: selectedServiceId,
        fullName: form.fullName,
        email: form.email,
        phone: form.phone || undefined,
        company: form.company || undefined,
        topic: selectedService.topic,
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/70 px-2">
              {isAr ? "اختيار الخدمة" : "Select Service"}
            </span>
            <div className="h-px flex-1 bg-border/30" />
          </div>
          ...
          <div className="space-y-1.5">
            <Label className="text-xs font-semibold text-foreground/70">
              {isAr ? "اختر الخدمة" : "Select a service"} <span className="text-red-400">*</span>
            </Label>
            <Select value={selectedServiceId} onValueChange={setSelectedServiceId}>
              <SelectTrigger className="h-11 rounded-xl text-sm bg-background border-border/60 focus:ring-2 focus:ring-[#7b57fc]/20 focus:border-[#7b57fc]/50 transition-all duration-200">
                <SelectValue
                  placeholder={isAr ? "اختر الخدمة التي تحتاجها…" : "Select the service you need…"}
                />
              </SelectTrigger>
              <SelectContent>
                {services.map((svc) => (
