...
const LOCATIONS = [
  {
    ...
    descEn: "Our largest team. We source from 500+ suppliers, visit factories, and run live market tours across southern China.",
    descAr: "أكبر فرقنا. نستورد من أكثر من ٥٠٠ مورد، نزور المصانع، ونجري جولات سوق مباشرة في جنوب الصين.",
  ...
  },
  {
    ...
    descEn: "Managing American supplier relationships, product quality verification, and US-to-MENA shipping coordination.",
    descAr: "إدارة علاقات الموردين الأمريكيين، التحقق من جودة المنتجات، وتنسيق الشحن من أمريكا إلى المنطقة العربية.",
  ...
  },
  {
    ...
    descEn: "Our client-facing team handling requests, consultations, and relationships across the GCC.",
    descAr: "فريقنا الموجه للعملاء يتولى الطلبات والاستشارات والعلاقات في منطقة الخليج.",
  ...
  },
  {
    ...
    descEn: "Logistics coordination, customs facilitation, and a regional hub for re-export and distribution.",
    descAr: "تنسيق اللوجستيك، تسهيل الجمارك، ومركز إقليمي لإعادة التصدير والتوزيع.",
  ...
  },
  {
    ...
    descEn: "Supporting Yemeni entrepreneurs with sourcing, imports, and trade consulting tailored to local market needs.",
    descAr: "دعم رواد الأعمال اليمنيين في الاستيراد والاستشارات التجارية المُكيَّفة مع احتياجات السوق المحلية.",
...
export function TeamPresence({ isAr }: { isAr: boolean }) {
  return (
    <section className="relative py-16 md:py-24 overflow-hidden" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="relative max-w-5xl mx-auto px-4 md:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#7b57fc]/10 border border-[#7b57fc]/20 mb-4">
            ...
            <span className="text-sm font-semibold text-[#7b57fc]">{isAr ? "فريقنا حول العالم" : "Our team worldwide"}</span>
          </div>
          <h2 className="text-2xl sm:text-3xl font-bold text-foreground leading-tight mb-3">
            {isAr
              ? <><span className="text-gradient">حاضرون في الميدان</span> لا خلف الشاشات فقط</>
              : <>We're <span className="text-gradient">on the ground</span>, not just behind screens</>
            }
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {LOCATIONS.map(({ flag, cityEn, cityAr, roleEn, roleAr, descEn, descAr, color }, i) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <span className={`fi fi-${code} text-3xl leading-none`}></span>
                  <div>
                    <p className="text-sm font-bold text-foreground leading-snug">{isAr ? cityAr : cityEn}</p>
                    <span className={cn("text-[10px] font-semibold px-2 py-0.5 rounded-full", color.split(" ").slice(0,2).join(" "))}>
                      <span className={color.split(" ").slice(2).join(" ")}>{isAr ? roleAr : roleEn}</span>
                    </span>
                  </div>
                </div>
